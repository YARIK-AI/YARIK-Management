...
chmod 400 host.key
openssl req -new -x509 -nodes -sha256 -days 365 -key host.key -out host.cert -subj "/C=RU/ST=Tatarstan/L=Kazan/O=TelInt/CN=www.example.com"
...
python -m gunicorn --bind "0.0.0.0:${RUN_PORT}" djtest.wsgi:application --daemon --log-level info --access-logfile /var/log/gunicorn/access.log --error-logfile /var/log/gunicorn/error.log
